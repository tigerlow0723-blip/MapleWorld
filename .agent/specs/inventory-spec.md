...
이전에 미구현 상태였던 핵심 시스템들이 현재 모두 코드로 완벽히 구현되어 적용 중입니다.

| 기능 | 현재 상태 | 설명 및 구현 위치 |
|------|----------|------|
| **비정형 아이템 크기** | ✅ 구현 완료 | `DataManager`의 `NormalizeShapeMask`를 통해 1x1, 2x2, `+`자형 등 비정형 행렬 완벽 지원. |
| **인벤토리 확장권** | ✅ 구현 완료 | `InventoryUI`에서 `expand` 타입 드래그 시 인접한 `-1` 칸 잠금 해제 빛 활성화 처리. |
| **아이템 회전** | ✅ 구현 완료 | `Item.mlua`의 `Rotate90Degrees()`로 90도 행렬 회전 및 엔티티 시각 효과 반전 처리. |
| **전투 스탯 연동** | ✅ 구현 완료 | `ItemManager.mlua`의 `GetTotalItemStats()`를 통해 가방 안의 아이템 스탯 총합을 전투에 실시간 적용. |
| **상점 및 보상 환전** | ✅ 구현 완료 | 가방 외부 배출, 잔여 보상 50골드 자동 판매 및 자유 테트리스 보관(Free Placement) 로직 지원. |
