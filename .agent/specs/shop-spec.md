...
상점은 열릴 때마다 4가지 등급 중 하나로 결정되며, **등급에 따라 정확히 정해진 품목 풀(Pool) 안에서만** 아이템 카드가 생성됩니다. (✅ 금의 원석 구매 제한은 `shop-addendum-spec.md`에서 구현 완료됨 — `goldOrePurchaseCount`로 추적)
...
4. **돼지고기(Pork) 즉시 소모 예외 처리 (신규)**:
   - 구매한 아이템(`purchasedItemId`)이 **11 (돼지고기)**인 경우, 인벤토리 대기열(`ItemDisplayArea`)에 생성하지 **않고** 즉시 캐릭터의 HP를 회복(`playerManager:Heal(hp)`) 시킵니다.
   - 이때 영문 로그를 필수로 출력합니다: `log("[ShopUI] Pork immediately consumed upon purchase. itemId: 11")`
   - 포션 등 다른 소모품이나 주문서는 대상이 아닙니다. 이 경우에만 `CreateItemEntity` 호출을 건너뜁니다.
5. **품절 처리**: 상점 진열대에 있던 그 아이템 엔티티는 파괴(품절)됩니다.
